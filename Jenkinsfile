    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=beta -v ${TEMPDIR}:/ansible/readme linuxserver/doc-builder:latest
              if [ "$(md5sum ${TEMPDIR}/${LS_REPO}/Jenkinsfile | awk '{ print $1 }')" != "$(md5sum Jenkinsfile | awk '{ print $1 }')" ] || \
                ...
                git --git-dir ${TEMPDIR}/repo/${LS_REPO}/.git checkout -f beta
    ...
    stage('Docker-Push-Multi') {
      ...
      steps {
        withCredentials([
          ...
          sh "docker tag ${IMAGE}:arm32v7-${META_TAG} ${IMAGE}:arm32v7-beta"
          ...
          sh "docker push ${IMAGE}:arm32v7-beta"
          sh "docker push ${IMAGE}:arm64v8-beta"
          sh "docker manifest push --purge ${IMAGE}:beta || :"
          sh "docker manifest create ${IMAGE}:beta ${IMAGE}:amd64-beta ${IMAGE}:arm32v7-beta ${IMAGE}:arm64v8-beta"
          sh "docker manifest annotate ${IMAGE}:beta ${IMAGE}:arm32v7-beta --os linux --arch arm"
